 ...
 * </pre>
 */
public class MyContigFilterServer extends JsonServerServlet {
    ...
    public MyContigFilterServer() throws Exception {
        super("MyContigFilter");
        //BEGIN_CONSTRUCTOR
     ...
     */
    @JsonServerMethod(rpc = "MyContigFilter.filter_contigs", async=true)
    ...
    @JsonServerMethod(rpc = "MyContigFilter.status")
    ...
    public static void main(String[] args) throws Exception {
        if (args.length == 1) {
            new MyContigFilterServer().startupServer(Integer.parseInt(args[0]));
        } else if (args.length == 3) {
            JsonServerSyslog.setStaticUseSyslog(false);
            JsonServerSyslog.setStaticMlogFile(args[1] + ".log");
            new MyContigFilterServer().processRpcCall(new File(args[0]), new File(args[1]), args[2]);
        } else {
