package MyContigFilter::MyContigFilterClient;
...
MyContigFilter::MyContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => MyContigFilter::MyContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a MyContigFilter.FilterContigsParams
$return is a MyContigFilter.FilterContigsResults
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	new_contigset_ref has a value which is a MyContigFilter.ws_contigset_id
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "MyContigFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for MyContigFilter::MyContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "MyContigFilter::MyContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package MyContigFilter::MyContigFilterClient::RpcClient;
